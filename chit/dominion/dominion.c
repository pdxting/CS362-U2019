...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	case mine:
		return mineAction(state, currentPlayer, choice1, choice2, handPos);
	...
	case baron:
		return baronAction(state, currentPlayer, choice1);
	...
	case minion:
		return minionAction(state, currentPlayer, choice1, choice2, handPos);
	...
	case tribute:
		return tributeAction(state, currentPlayer, nextPlayer);

	case ambassador:
		return ambassadorAction(state, currentPlayer, choice1, choice2, handPos);
...
int mineAction(struct gameState * state, int currentPlayer, int choice1, int choice2, int handPos)
{
	...
	int cardToTrash = state->hand[currentPlayer][choice2];  //*****BUG: choice1 changed to choice2
	...
	for (int i = 0; i < state->handCount[currentPlayer]; i++)
	{
		if (state->hand[currentPlayer][i] = cardToTrash)//*****BUG: state->hand[currentPlayer][i] == cardToTrash
		{
			discardCard(i, currentPlayer, state, 0);
		}
	}
...
int tributeAction(struct gameState * state, int currentPlayer, int nextPlayer)
{
	...
	if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
		if (state->deckCount[nextPlayer] > 0) {
			card1 = state->deck[nextPlayer][state->deckCount[nextPlayer] - 1];
		...
		card1 = setCard(state, nextPlayer, card1);
		card2 = setCard(state, nextPlayer, card1); //*****BUG:card2 = setCard(state, nextPlayer, card2);
	}

	if (card1 == card2) {//If we have a duplicate card, just drop one 
		state->playedCards[state->playedCardCount] = card2;
		state->playedCardCount++;
		card2 = -1;
	}

	giveCardToPlayer(state, currentPlayer, card1);
	giveCardToPlayer(state, currentPlayer, card2);
...
{
	card = state->deck[nextPlayer][state->deckCount[nextPlayer] - 1];
...
{
	if (card == copper || card == silver || card == gold) {//Treasure cards, give player two copper
		state->coins += 2;
	}
	else if (card == estate || card == duchy || card == province || card == gardens || card == great_hall) {//Victory Card Found, give player two more cards
		drawCard(currentPlayer, state);
		//drawCard(currentPlayer, state);//*****BUG: One of the statement is commented out.
	}
	else {//Action Card, player gains two more actions
		state->numActions = state->numActions + 2;
	}
...
{
	int remainingCardCount = 0;		//used to check if player has enough cards to discard
	...
	for (int i = 0; i < state->handCount[currentPlayer]; i++)
	{
		if (i != handPos || i == state->hand[currentPlayer][choice1] || i != choice1) //*****BUG: i != handPos && i == state->hand[currentPlayer][choice1] && i != choice1
		{
			remainingCardCount++;
		}
	}
	if (remainingCardCount < choice2)
	{
		return -1;
	}
	...
	for (int i = 0; i < state->numPlayers; i++)
	{
		if (i == currentPlayer) //*****BUG: i != currentPlayer is now i == currentPlayer
		{
	...
	if (choice1 > 0) {//Boolean true or going to discard an estate
		...
		while (estateNotFound) {
			if (state->hand[currentPlayer][currentCard] == estate) {//Found an estate card!
				...
				state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][currentCard];
				state->discardCount[currentPlayer]++;
				for (; currentCard < state->handCount[currentPlayer]; currentCard++) { 
					state->hand[currentPlayer][currentCard] = state->hand[currentPlayer][currentCard + 1];
				}
...
int minionAction(struct gameState * state, int currentPlayer, int choice1, int choice2, int handPos)
{
	...
	//discard card from hand
	//discardCard(handPos, currentPlayer, state, 0); //*****BUG: discardCard commented out
		...
		for (int i = 0; i < state->numPlayers; i++)
		{
			if (i != currentPlayer)
			{
				if (state->handCount[i] > 4)
				{
					...
					for (int j = 0; j < 4; j++)
					{
						drawCard(j, state); //***** BUG: drawCard(i, state) is now drawCard(j, state)
					}
				}
			}
		}
