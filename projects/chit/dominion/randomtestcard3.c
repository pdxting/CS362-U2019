...
int main() {
	...
	for (int i = 0; i < 1000; i++) {
		...
		tributeRevealedCards[0] = beforeState.deck[nextPlayer][beforeState.deckCount[nextPlayer] - 1];
		tributeRevealedCards[1] = beforeState.deck[nextPlayer][beforeState.deckCount[nextPlayer] - 2];
		...
		if (beforeState.discardCount[nextPlayer] + beforeState.deckCount[nextPlayer] <= 1) {
			...
			assertCheck(afterState.deck[nextPlayer][afterState.deckCount[nextPlayer] - 1], tributeRevealedCards[0]);
			printf("card2 before and after function call. ");
			assertCheck(afterState.deck[nextPlayer][afterState.deckCount[nextPlayer] - 2], tributeRevealedCards[1]);
		}
